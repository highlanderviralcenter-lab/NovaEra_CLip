...
class ClipFusionApp:
    ...
    def _tab_ia(self):
        ...
        self._lbl(f, "Cole no Claude.ai, ChatGPT ou qualquer IA. Traga o JSON de resposta abaixo.", color=GRY).pack(anchor="w", padx=30)
        self.box_prompt = scrolledtext.ScrolledText(f, height=11, bg=BG3, fg="#a5b4fc", font=MONO, relief="flat", insertbackground=WHT)
        self.box_prompt.pack(fill="x", padx=30, pady=(4,14))
        self._lbl(f, "Resposta da IA (cole o JSON aqui):", font=FNTB).pack(anchor="w", padx=30)
        self.box_resp = scrolledtext.ScrolledText(f, height=13, bg=BG3, fg=GRN, font=MONO, relief="flat", insertbackground=WHT)
    ...
    def _tab_agenda(self):
        ...
        for p in ["tiktok","instagram","youtube","kwai"]:
            tk.Radiobutton(cfg, text=p, variable=self.v_platform, value=p, bg=BG2, fg=WHT, selectcolor=ACC, activebackground=BG2, font=FNT).pack(side="left", padx=8)
        ...
        tk.Entry(cfg2, textvariable=self.v_count, width=6, bg=BG3, fg=WHT, insertbackground=WHT, relief="flat", font=FNT).pack(side="left", padx=10)
    ...
    def _select_video(self):
        p = filedialog.askopenfilename(title="Selecionar vídeo", filetypes=[("Vídeos","*.mp4 *.mkv *.mov *.avi *.webm"),("Todos","*.*")])
        if p:
            self.video_path = p
    ...
    def _draw_cut_card(self, cut, cid, archetypes):
        ...
        tk.Checkbutton(hdr, variable=var, bg=BG3, fg=WHT, selectcolor=ACC, activebackground=BG3, font=FNTB).pack(side="left")
        dur = cut["end"] - cut["start"]
        arch_id = cut.get("archetype", "")
        arch_info = archetypes.get(arch_id, {})
        emoji = arch_info.get("emoji", "✂️")
        tk.Label(hdr, text=f"{emoji} {cut.get('title','Corte')}", bg=BG3, fg=WHT, font=FNTB).pack(side="left", padx=4)
        tk.Label(hdr, text=f"  {fmt_time(cut['start'])} → {fmt_time(cut['end'])}  ({fmt_time(dur)})", bg=BG3, fg=GRY, font=FNT).pack(side="left")
        tk.Label(hdr, text=f" {arch_id} ", bg=ACC, fg=WHT, font=("Segoe UI",8,"bold"), padx=5, pady=1).pack(side="right")
        tk.Label(hdr, text=", ".join(cut.get("platforms",[])), bg=BG3, fg=YEL, font=("Segoe UI",9)).pack(side="right", padx=8)
        if cut.get("hook"):
            tk.Label(card, text=f"🎣  {cut['hook']}", bg=BG3, fg="#a5b4fc", font=FNT, wraplength=960, justify="left", anchor="w").pack(fill="x", padx=22, pady=2)
        if cut.get("reason"):
            tk.Label(card, text=f"💡  {cut['reason']}", bg=BG3, fg=GRY, font=FNT, wraplength=960, justify="left", anchor="w").pack(fill="x", padx=22, pady=(0,8))
                ...
                for cut in approved:
                    if cut.get('archetype') and cut.get('title'):
                        try:
                            ...
                            hook_data = ViralHookEngine().generate(tema=cut['title'], nicho='geral', platform='tiktok', archetype_id=cut['archetype'])
                            self.root.after(0, lambda m=f"🎯 Gancho: {hook_data['gancho_final'][:50]}": self._log(m))
                        except:
                            pass
                results = render_all(vid, approved, segs, out_dir, pid, ace_level=ace, use_vaapi=vaapi, progress_cb=lambda m: self.root.after(0, lambda msg=m: self._log(msg)))
    ...
    def _lbl(self, p, text="", font=None, color=None):
        bg_color = p.cget("bg") if hasattr(p, "cget") else BG2
        return tk.Label(p, text=text, bg=bg_color, fg=color or WHT, font=font or FNT)

    def _btn(self, p, text, cmd, color=BG3, wide=False):
        return tk.Button(p, text=text, command=cmd, bg=color, fg=WHT, font=FNTB, relief="flat", cursor="hand2", padx=20 if wide else 14, pady=8, activebackground=color, activeforeground=WHT, width=50 if wide else None)

    def _chk(self, p, text, var):
        return tk.Checkbutton(p, text=text, variable=var, bg=p.cget("bg"), fg=WHT, selectcolor=ACC, activebackground=p.cget("bg"), font=FNT)
