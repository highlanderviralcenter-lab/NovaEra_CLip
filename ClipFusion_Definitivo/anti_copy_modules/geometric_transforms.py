...
class GeometricTransforms:
    ...
    def ffmpeg_filters(self, rotation: bool = False, perspective: bool = False) -> list:
        ...
        if rotation: rad = self.rng.uniform(-0.3, 0.3) * 3.14159/180; filters.append(f"rotate={rad:.5f}:fillcolor=black:expand=0")
        if perspective:
            ox = self.rng.uniform(0.001, 0.003); oy = self.rng.uniform(0.001, 0.003)
            filters.append(f"perspective=x0=iw*{ox:.4f}:y0=ih*{oy:.4f}:x1=iw*(1-{ox:.4f}):y1=ih*{oy:.4f}:x2=iw*{ox:.4f}:y2=ih*(1-{oy:.4f}):x3=iw*(1-{ox:.4f}):y3=ih*(1-{oy:.4f}):interpolation=linear")
        return filters
